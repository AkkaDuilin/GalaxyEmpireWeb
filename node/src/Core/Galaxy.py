...
def show_server_list() -> dict:
    """Fetch and process the server list."""
    ...
    try:
        ...
        logging.warning(f"Failed to fetch server list: {e}")
        return {}

    server = {}
    for item in data['lists']:
        name = item['name'].split(':')[0] if ':' in item['name'] else item['name'][:2].lower()
        server[name] = item['url']
    return server
...
class Galaxy(GalaxyCore):
    def __init__(self):
        ...
        self.fleet_on_planet = defaultdict(dict)
        self.resources_on_planet = defaultdict()
        self.planet_in_danger = []
        self.logger = None
        ...
        try:
            server_lst = show_server_list()
            self.server_url_list = server_lst
        except Exception as e:
            logging.warning(f"Error fetching server list: {e}")
        # self.startup()
